...
export class GitService {
    ...
    async createPatchAndPR(
        ...
        try {
            ...
            // 2. Get current file content and SHA for first file
            const filePath = diff.filesTouched[0]; // Take first file for now
    ...
    private applyDiff(content: string, diff: CodeDiff): string {
        // For now, return the original content with diff.patch as comment
        // In production, would:
        // 1. Parse the diff properly using a patch library
        // 2. Apply line-by-line changes
        // 3. Handle conflicts
        // Since we don't have actual patch content to apply, return original
        return content + "\\n\\n// Diff: " + diff.patch.substring(0, 100);
    }
